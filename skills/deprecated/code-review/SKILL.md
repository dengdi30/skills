...
| 条件 | 派发 |
|------|------|
| 必选（每次） | `Task(general_purpose_task)` + `code-quality-reviewer-prompt.md` |
| 有 task_text | `Task(general_purpose_task)` + `spec-reviewer-prompt.md` |
| diff 含 `.py` 文件 | `Task(general_purpose_task)` + `python-reviewer-prompt.md` |
| diff 含 `.ts` / `.tsx` 文件 | `Task(general_purpose_task)` + `typescript-reviewer-prompt.md` |
...
**派发：** `Task(general_purpose_task)` + `global-reviewer-prompt.md`
...
Trae 不支持在 Task 工具中指定模型，所有 reviewer subagent 使用 Trae 默认模型承担。

| 派发 | rune 原模型 | wyrd 模型 | 备注 |
|------|------|------|------|
| code-quality-reviewer | opus | Trae 默认 | general_purpose_task，含安全审查 |
| spec-reviewer | sonnet | Trae 默认 | general_purpose_task |
| python-reviewer | sonnet | Trae 默认 | general_purpose_task + prompt 模板 |
| typescript-reviewer | sonnet | Trae 默认 | general_purpose_task + prompt 模板 |
| global-reviewer | opus | Trae 默认 | general_purpose_task |
