---
name: review
description: 当需要对工作区、commit、分支、PR 或用户提供的 diff 进行独立代码审查时使用。通过不继承当前对话和实现过程的 fresh subagent，对确定的候选变更进行只读、证据驱动的对抗性审查，输出绑定候选的 APPROVE 或 BLOCK 结论；不修改代码。
---
...
本 skill 可以独立使用。它不要求需求文档、Plan、实现报告或测试证据存在，也不调用或依赖其他 skills。
...
只返回绑定 candidate 的审查结论，由调用者决定下一步。
